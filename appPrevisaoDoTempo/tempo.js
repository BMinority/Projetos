...
form.addEventListener("submit", function (event) {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      changeTemperatureColor(data.current.temp_c);
      document.getElementById('temperature').style.backgroundColor = 'rgba(0, 0, 0, 0.684)';
      document.getElementById('temperature').style.borderRadius = '10px';
      document.getElementById('temperature').style.fontWeight = 'bold';
      document.getElementById('temperature').style.padding = '10px';
...
function changeTemperatureColor(temp) {
  const tempPara = document.getElementById("temperature");
  
  if (temp >= 30) {
    tempPara.style.color = "red";
  } else if (temp > 20) {
    tempPara.style.color = "orange";
  } else if (temp > 16) {
    tempPara.style.color = "yellow";
  } else if (temp >= 0) {
    tempPara.style.color = "lightblue";
  } else {
    tempPara.style.color = "blue";
  }
}  
